/*Моделирование воды. 
Основная идея земля и дно представляются в виде карты высот, которые берутся из текстур (см.ниже) по ним течет вода прибой модерируеся по заранее расчитаным формулам
*/
#include <cstdlib>
#include <cstdio>
#include <cstdarg>
...
/*stb_image.h — это очень популярная библиотека загрузки изображений, написанная Шоном Барреттом, и при этом состоящая всего из одного заголовочного файла, которая способна загружать самые популярные форматы файлов (и её легко интегрировать в ваш проект(ы)). */
...
/*базовая струтура хранения информации (позиция нормаль текстурная координата - т.е. указание как натягивать текстуру*/
struct Vertex {
	Vec3 position;
	Vec3 normal;
	Vec2 texCoord;
};
// базовые настройки видео карты
...
// что отрисовываем упрвление по кнопкам F1 - F8
...
// базовая структура с данными кадра
...
// проверка отработки видеокартой шейдера
...
// Вызов базовых настороек OPENGL для обработки шейдера
...
// сама программа предствляет собой набор подпрограмм 1 с отрисокой воды 2 с отрисовкой земли 3  с моделированием волн неба воды и текстур разница в том что при нажатии на кнопки F1-
struct Program {
	GLuint id;
...
/*Главный класс с поверхностью все обекты состоят из воверхностей*/
class Mesh {
public:
...
Насколько я смог разобраться, то, что лежит в VAO по каким-то индексам, правильно называется «vertex attribute array». Название часто сокращают до «vertex attributes» или просто «attributes». Соответственно, отдельный элемент массива называется атрибутом (attribute, без s на конце). При этом каждый атрибут состоит из компонетов (components). Например, координаты вершины (один атрибут) задаются тремя координатами (три компонента типа float). Чтобы нарисовать треугольник, нужно задать три вершины, поэтому нужен массив из трех атрибутов. Усложняется ситуация тем, что авторы туториалов могут использоваться свою собственную терминологию, вроде attribute list или подобную. Еще мне встречались просто опечатки, когда в тексте написано attribute, когда по контексту явно должно быть attributes. Стоит ли говорить, что такой бардак несколько усложняет изучение OpenGL? :)*/
	...
	// когда всё расчитали вызываем эту функцию и отрисовываем
	void render() const {
...
GLFWwindow* createContext() {
	...
	// виндовое окно и команды управления
...
int main(int argc, char* argv[]) {
	...
	appData.initialize(); // функция иеициализации экранной области

	initializeUnitQuad();  // структура в которой хранятся данные о графических примитивах

	int framebufferWidth, framebufferHeight; 
	glfwGetFramebufferSize(appData.window, &framebufferWidth, &framebufferHeight);
	appData.projectionMatrix = glm::perspectiveFov(45.0f, float(framebufferWidth), float(framebufferHeight), 0.001f, 100.0f); // матрица преобразования
	...
	// Тестура
	appData.debugTexture = importTexture("../../content/textures/debugTexture1.png"); // дно
	appData.noiseTexture = importTexture("../../content/textures/noiseTexture.png"); // шум с этой текстуры читаются волны
	appData.noiseNormalTexture = importTexture("../../content/textures/noiseNormalTexture.jpg"); // нормали для волн
	appData.causticTexture = importTexture("../../content/textures/causticTexture.png");  // "едкая текстура " для при дания глубины
	appData.subsurfaceScatteringTexture = createSubsurfaceScatteringTexture(); // текстура подповерхностного рассеяния
	...
	Mesh groundMesh(terrainSize, groundHeightFunction); // поверхность земли

	Mesh waterMesh(terrainSize, [](Vec2 coordinate) { //// поверхность воды 
		return 0;
	});

	FpsCounter fpsCounter; // счётчик кадров

	glClearColor(0, 0.5f, 0.5f, 1); // базовый цвет окна
	glClearDepth(1); // буфер глубины
	glEnable(GL_DEPTH_TEST); // каманда орисовки
